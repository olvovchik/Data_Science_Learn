...
with col1:
    x2 = st.number_input('Плотность, кг/м3', min_value = 0., step = 1., format='%.2f', value = 1881.62)
    x3 = st.number_input('Модуль упругости, ГПа', min_value = 0., step = 1., format='%.2f', value = 663.33)
    x4 = st.number_input('Количество отвердителя, м.%', min_value = 0., step = 1., format='%.2f', value = 123.51)
    x5 = st.number_input('Содержание эпоксидных групп,%_2', min_value = 0., step = 1., format='%.2f', value = 28.32)
    x6 = st.number_input('Температура вспышки, С_2', min_value = 0., step = 1., format='%.2f', value = 220.23)


with col2:    
    x7 = st.number_input('Поверхностная плотность, г/м2', min_value = 0., step = 1., format='%.2f', value = 363.13)
    x10 = st.number_input('Потребление смолы, г/м2', min_value = 0., step = 1., format='%.2f', value = 314.70)
    x11 = st.selectbox('Угол нашивки, град', [0, 90])
    x12 = st.number_input('Шаг нашивки', min_value = 0., step = 1., format='%.2f', value = 9.59)
    x13= st.number_input('Плотность нашивки', min_value = 0., step = 1., format='%.2f', value = 35.88)
